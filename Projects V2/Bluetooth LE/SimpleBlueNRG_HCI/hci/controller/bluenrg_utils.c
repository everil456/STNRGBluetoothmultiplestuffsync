...
int program_IFR(const IFR_config_TypeDef *ifr_image)
{
  ...
  ************************************************************************/  
  ...
  while (num_erase_retries++ < MAX_ERASE_RETRIES) {
    ...
    for(int i = 0; i < SECTOR_SIZE; i += DATA_SIZE) {
      RETRY_COMMAND(aci_updater_program_data_block(IFR_BASE_ADDRESS+i, DATA_SIZE, ifr_data+i), MAX_WRITE_RETRIES, ret);
      if (ret != BLE_STATUS_SUCCESS)
	break;
    }
    if (ret == BLE_STATUS_SUCCESS)
  ...
  ************************************************************************/  
  {
	...
	return ret;
      ...
      if (memcmp(ifr_updated, ifr_data+i, DATA_SIZE) != 0)
