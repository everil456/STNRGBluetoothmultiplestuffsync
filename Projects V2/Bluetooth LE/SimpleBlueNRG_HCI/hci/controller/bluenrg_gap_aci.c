...
tBleStatus aci_gap_delete_ad_type(uint8_t ad_type)
{
  ...
  rq.cparam = &cp;
  rq.clen = GAP_DELETE_AD_TYPE_CP_SIZE;
  rq.rparam = &status;
  rq.rlen = 1;
				     ...
				     uint16_t max_conn_length)
{
  ...
  rq.cparam = &cp;
  rq.clen = GAP_START_NAME_DISCOVERY_PROC_CP_SIZE;
  rq.event = EVT_CMD_STATUS;
  rq.rparam = &status;
  rq.rlen = 1;
                         ...
                         const uint8_t *addr_array)
{
  ...
  if (((num_whitelist_entries*7)+25) > HCI_MAX_PACKET_SIZE)
    return BLE_STATUS_INVALID_PARAMS;
						 ...
						 uint8_t own_address_type, uint8_t filter_duplicates, uint8_t use_reconn_addr, const tBDAddr reconn_addr)
{
  ...
  rq.cparam = &cp;
  rq.clen = GAP_START_GENERAL_CONN_ESTABLISH_PROC_CP_SIZE;
  rq.event = EVT_CMD_STATUS;
  rq.rparam = &status;
  rq.rlen = 1;
                         ...
                         const uint8_t *addr_array)
{
  ...
  if (((num_whitelist_entries*7)+GAP_START_SELECTIVE_CONN_ESTABLISH_PROC_CP_SIZE) > HCI_MAX_PACKET_SIZE)
    return BLE_STATUS_INVALID_PARAMS;
                                           ...
                                           uint16_t max_conn_length)
{
  ...
  rq.cparam = &cp;
  rq.clen = GAP_START_CONNECTION_UPDATE_CP_SIZE;
  rq.event = EVT_CMD_STATUS;
  rq.rparam = &status;
  rq.rlen = 1;
...
tBleStatus aci_gap_send_pairing_request(uint16_t conn_handle, uint8_t force_rebond)
{
  ...
  rq.cparam = &cp;
  rq.clen = GAP_GAP_SEND_PAIRING_REQUEST_CP_SIZE;
  rq.event = EVT_CMD_STATUS;
  rq.rparam = &status;
  rq.rlen = 1;
...
tBleStatus aci_gap_resolve_private_address(const tBDAddr address)
{
  ...
  rq.cparam = &cp;
  rq.clen = GAP_RESOLVE_PRIVATE_ADDRESS_CP_SIZE;
  rq.event = EVT_CMD_STATUS;
  rq.rparam = &status;
  rq.rlen = 1;
